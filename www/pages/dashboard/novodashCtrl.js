angular.module("eurecaJovem").controller("novodashCtrl", function ($route, $mdDialog, $sce, $http, $scope, $routeParams, $location, $mdToast, $mdSidenav, $rootScope, $sessionStorage, eurecaJovemAPI, eurecaJovemStorageAPI) {

    console.log("Perfil controler");
    $scope.checaPreenchido = function(card, valor) {
        console.log("Card para true", card);
        if (valor != null) {
            $rootScope.statusPreenchido.card = valor;
            console.log("status preenchido", $rootScope.statusPreenchido);
        }
    }
    ...
    $scope.carregarPerfil = function () {
        ...
        eurecaJovemAPI.buscarPessoaPorIdService($rootScope.usuarioLogado.id).then(function (resposta) {
            if (resposta.status == 200) {
                ...
                if (pessoa.idiomas[0]) {
                    console.log("idiomas veio", pessoa.idiomas);
    ...
    $scope.cadastrarInfopessoal = function () {
    ...
    $scope.cadastrarExperiencia = function (tipo) {
        $rootScope.pessoa.qualificacao = {};
        if (tipo != null)
            $rootScope.pessoa.qualificacao.tipo = tipo;
    ...
    $scope.alterarIdioma = function () {
        if (!angular.isUndefined($rootScope.pessoa.idioma.idIdioma)) {
            ...
            if (!$rootScope.pessoa.idiomas) {
                $rootScope.pessoa.idiomas = [];
            }
    ...
    //enuns
    $rootScope.generos = [{ valor: "0", descricao: "Não informado" },
    ...
    $rootScope.cursos = [{ valor: "0", curso: "Administração em Comércio Exterior" },
    ...
    $rootScope.tipoQualificacao = [{ valor: "0", descricao: "Formação acadêmica" },
    ...
    $scope.salvarIdiomas = function () {
        ...
        eurecaJovemAPI.perfilIdiomasService(jovemEdit).then(function (resposta) {
            if (resposta.status == 200) {
                $scope.checaPreenchido('idioma', true);
                $route.reload();
    ...
    $scope.salvarHabilidades = function () {
        ...
        eurecaJovemAPI.perfilHabilidadesService(jovemEdit).then(function (resposta) {
            if (resposta.status == 200) {
                $scope.checaPreenchido('habilidade', true);
                $route.reload();
                $mdToast.show($mdToast.simple().textContent(resposta.data));
    ...
    $scope.salvarApreciacoes = function () {
        ...
        eurecaJovemAPI.perfilApreciacoesService(jovemEdit).then(function (resposta) {
            if (resposta.status == 200) {
                $scope.checaPreenchido('apreciacao', true);
                $route.reload();
                $mdToast.show($mdToast.simple().textContent(resposta.data));
                $mdSidenav('sideApreciacao').close();
            } else {
    ...
    $scope.salvarInfosPessoais = function () {
        ...
        eurecaJovemAPI.perfilInfosPessoaisService(jovemEdit).then(function (resposta) {
            if (resposta.status == 200) {
                $scope.checaPreenchido('perfil', true);
                $route.reload();
                $mdToast.show($mdToast.simple().textContent(resposta.data));
    ...
    $scope.salvarQualificacao = function () {
        ...
        eurecaJovemAPI.perfilQualificacoesService(jovemEdit).then(function (resposta) {
            if (resposta.status == 200) {
                $scope.checaPreenchido('qualificacao', true);
                $route.reload();
                $mdToast.show($mdToast.simple().textContent(resposta.data));
    ...
    //controles do select de Grupo Curso
    $scope.removeItemList = function (item, list) {
        //console.log("Removendo imagem da lista: ", item, list);
        var idx = list.indexOf(item);
        list.splice(idx, 1);
    };

    //controles do select de Grupo Curso
    $scope.addItemList = function (item, list) {
        list.push(item);
    };
